...
> 7 个模型 GPU 对比，3 个模型 CPU 对比。184 条中文长音频（共 192 分钟）。[完整评测报告](https://modelscope.github.io/FunASR/zh/benchmark.html)

### GPU 结果

| 模型 | 类型 | 速度 | 说明 |
|------|------|------|------|
| **SenseVoice-Small** | 非自回归 | **170x** 实时 | 最快 + 最准 |
| **Paraformer-Large** | 非自回归 | **120x** 实时 | 速度精度最佳平衡 |
| Whisper-large-v3-turbo (OpenAI) | 自回归 | 46x 实时 | 最快 Whisper 版本 |
| Whisper-large-v3-turbo (FunASR) | 自回归 | 26x 实时 | 含 VAD 流水线 |
| faster-whisper-large-v3 | 自回归 | 21.5x 实时 | CTranslate2 优化 |
| **Fun-ASR-Nano** | 大模型 | 17x 实时 | 大模型最高精度 |
| Whisper-large-v3 (OpenAI) | 自回归 | 13.4x 实时 | 基线 |

### CPU 结果

| 模型 | 速度 | 说明 |
|------|------|------|
| **SenseVoice-Small** | **17.2x** 实时 | CPU 生产可用 |
| **Paraformer-Large** | **15.6x** 实时 | CPU 生产可用 |
| **Fun-ASR-Nano** | 3.6x 实时 | 仍超实时 |
| Whisper 系列 | - | CPU 不实用（192分钟音频需2小时+） |

<a name="最新动态"></a>

## 最新动态

- 2026/05/24: **v1.3.2 发布** — 修复 HuggingFace 用户 `hub="hf"` 参数传播问题；新增[全面性能评测](https://modelscope.github.io/FunASR/zh/benchmark.html)（7 个 GPU 模型 + 3 个 CPU 模型对比）。`pip install --upgrade funasr`
