...
from fun_text_processing.inverse_text_normalization.zh.graph_utils import DAMO_ALPHA, GraphFst, insert_space
...
class ElectronicFst(GraphFst):
    ...
    def __init__(self):
        ...
        graph = username + delete_extra_space + (pynutil.delete("at")|pynutil.delete("艾特")) + insert_space + delete_extra_space + domain_graph
        ...
        protocol = (
            pynini.closure(protocol_start, 0, 1) + protocol_end + pynini.closure(pynini.cross(" ", ""), 0, 1) + process_dot + pynini.closure(pynini.cross(" ", ""), 0, 1) + protocol_default
        ).optimize()
