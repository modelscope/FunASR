...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        graph_one_hundred_component = pynini.union(pynini.cross("ratus", "1") | pynini.cross("seratus", "1"))
        ...
        graph_billion = pynini.union(
            graph_hundred_component_at_least_one_none_zero_digit + delete_space + (pynutil.delete("miliar") | pynutil.delete("semiliar") | pynutil.delete("milyar") | pynutil.delete("semilyar")),
            pynutil.insert("000", weight=0.1),
        )
        graph_trillion = pynini.union(
            graph_hundred_component_at_least_one_none_zero_digit + delete_space + (pynutil.delete("triliun") | pynutil.delete("setriliun")),
            pynutil.insert("000", weight=0.1),
        )
        graph_quadrillion = pynini.union(
            graph_hundred_component_at_least_one_none_zero_digit + delete_space + pynutil.delete("milion lipat empat"),
            pynutil.insert("000", weight=0.1),
        )
        graph_quintillion = pynini.union(
            graph_hundred_component_at_least_one_none_zero_digit + delete_space + pynutil.delete("triliun"),
            pynutil.insert("000", weight=0.1),
        )
        ...
        graph = (
                pynini.cdrewrite(pynutil.delete("dan"), DAMO_SPACE, DAMO_SPACE,
                                 DAMO_SIGMA)
