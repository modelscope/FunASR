...
def test_llama_cpp_pages_render_v025_download_matrix(built_site, relative, boundary):
    soup = read_soup(built_site / relative)
    ...
    assert len(rows) == 9
    assert all(row.select_one('a[href*="runtime-llamacpp-v0.2.5"]') for row in rows)
