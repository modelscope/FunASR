...
class SenseVoiceLPitch(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.loss_f0_weight = kwargs.get("loss_f0_weight", 0.3)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.activation_checkpoint:
            ...
            encoder_out, encoder_f0_out, encoder_out_lens = self.encode(speech, speech_lengths)
        ...
        loss_f0 = self._cal_f0_loss(
            encoder_f0_out, speech_lengths, f0, f0_tag
        )
        
        loss = loss_att + loss_f0 * self.loss_f0_weight
        stats = {}
        stats["acc"] = acc_att
        stats["loss"] = torch.clone(loss.detach())
        stats["batch_size"] = batch_size
    ...
    def _cal_f0_loss(self, encoder_f0_out, speech_lengths, f0, f0_tag):
        ...
        padding_mask = padding_mask * f0_tag.unsqueeze(1) # B*T * B*1
        
        f0_loss = torch.abs(f0 - encoder_f0_out.squeeze()) * padding_mask
        f0_loss = f0_loss.mean()
        return f0_loss
    ...
    def encode(
        ...
        **kwargs,
        ...
        # Forward encoder
        encoder_out, encoder_f0_out, encoder_out_lens = self.model.encoder(speech.permute(0, 2, 1), speech_lengths)

        return encoder_out, encoder_f0_out, encoder_out_lens
