...
class MelSpectrumExtractor(nn.Module):
    ...
    def convert_power_to_mel(self, x, ilens):
        feat = mel_from_power_spectrogram(
            x,
            self.n_fft,
            self.num_mels,
            self.sampling_rate,
...
class QuantizerCodebook(torch.nn.Module):
    def __init__(self, num_quantizers, codebook_size, codebook_dim, hop_length, sampling_rate):
        super().__init__()
    ...
    def forward(self, codec: torch.Tensor, codec_lengths, return_subs=False):
        ...
        codec_mask = (
            ~make_pad_mask(codec_lengths, maxlen=codec.shape[1]).unsqueeze(-1).to(codec.device)
        )
...
class BaseDiffWithXvec(nn.Module):
        ...
        **kwargs,
        ...
        if output_type == "mel":
            ...
            self.quantizer_codebook = QuantizerCodebook(
                num_quantizers, codebook_size, codebook_dim, hop_length, sampling_rate
            )
    ...
    def select_target_prompt(self, y, y_lengths):
        ...
        if "cgf_prob" in prompt_conf and prompt_conf["cgf_prob"] > 0:
            cgf_mask = (
                torch.rand([y.shape[0], 1, 1], dtype=torch.float32, device=y.device)
                < prompt_conf["cgf_prob"]
            )
    ...
    def build_length_regulator(self):
        ...
        if name == "upsampling":
            ...
            model = UpSamplingRegulator(
                self.output_size, self.length_regulator_conf.get("sampling_ratios")
            )
        elif name == "downsampling":
            from funasr.models.llm_asr.diffusion_models.length_regulator import (
                DownSamplingRegulator,
            )

            model = DownSamplingRegulator(
                self.output_size, self.length_regulator_conf.get("sampling_ratios")
            )
    ...
    @torch.no_grad()
    def inference(
        self, text, text_lens, xvec, xvec_lens, diff_steps=10, temperature=1.0, prompt=None
    ):
        ...
        if self.output_type == "mel":
            coeff = (
                self.mel_extractor.sampling_rate / self.mel_extractor.hop_size
            ) / self.input_frame_rate
        else:
            coeff = (
                self.quantizer_codebook.sampling_rate / self.quantizer_codebook.hop_size
            ) / self.input_frame_rate
...
class MaskedDiffWithXvec(BaseDiffWithXvec):
    def __init__(
        ...
        **kwargs,
        ...
        if self.io_ratio == "auto":
            self.io_ratio = (
                mel_feat_conf["sampling_rate"] / mel_feat_conf["hop_size"] / self.input_frame_rate
            )
    ...
    def sample_first_package(
        ...
        if random_start:
            ...
            for i, total_len in enumerate(text_lengths):
                ...
                if sample_mask[i].item():
                    ...
                    if total_len > max_token_len > min_token_len:
                        ...
                        audio_st, audio_len = self.calc_target_len(
                            torch.tensor(start)
                        ), self.calc_target_len(torch.tensor(fp_len))
                    else:
                    ...
                    audio_list.append(
                        audio[i, : min(self.calc_target_len(text_lengths[i]), audio_lengths[i])]
                    )
                    audio_lengths_list.append(audio_list[-1].shape[0])
            text = pad_list(text_list, pad_value=0.0).to(text)
            new_text_lengths = torch.tensor(
                text_lengths_list, dtype=torch.int64, device=text.device
            )
            audio = pad_list(audio_list, pad_value=0.0).to(audio)
            new_audio_lengths = torch.tensor(
                audio_lengths_list, dtype=torch.int64, device=audio.device
            )
        else:
            ...
            text = text * (~make_pad_mask(new_text_lengths, maxlen=text.shape[1]).unsqueeze(-1)).to(
                text.device
            )
            audio = audio * (
                ~make_pad_mask(new_audio_lengths, maxlen=audio.shape[1]).unsqueeze(-1)
            ).to(audio.device)
        ...
        with autocast(False):
            ...
            if self.length_normalizer_ratio is not None:
                ...
                if y_lengths.max() > max_y_lengths.max():
                    logging.info(
                        f"cut output with ratio {self.io_ratio} from {y_lengths.max()} to {max_y_lengths.max()}"
                    )
                    y_lengths = torch.where(y_lengths > max_y_lengths, max_y_lengths, y_lengths)
                    y = y[:, : y_lengths.max()]
