...
@tables.register("dataset_classes", "SenseVoicePitchDataset")
class SenseVoicePitchDataset(torch.utils.data.Dataset):
    """
    SenseVoiceDataset
    """
    ...
    def __getitem__(self, index):
        ...
        for idx in range(self.retry):
            ...
            if 'f0' in item:
                f0_file = item['f0']
                f0 = torch.tensor(np.load(f0_file), dtype=torch.float32)
                if f0.shape > speech.shape[1]:
                    f0 = f0[:speech.shape[1]]
