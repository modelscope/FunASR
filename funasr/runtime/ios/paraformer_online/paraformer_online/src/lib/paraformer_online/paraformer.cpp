...
void Paraformer::InitAsr(const std::string &am_model, const std::string &am_cmvn, const std::string &am_config, int thread_num){
    ...
    try {
        ...
        std::cout << "Successfully load model from " << am_model << std::endl;
    } catch (std::exception const &e) {
        std::cout << "Error when load am onnx model: " << e.what() << std::endl;
        exit(0);
    }
...
void Paraformer::InitAsr(const std::string &en_model, const std::string &de_model, const std::string &am_cmvn, const std::string &am_config, int thread_num){
    ...
    try {
        ...
        std::cout << "Successfully load model from " << en_model << std::endl;
    } catch (std::exception const &e) {
        std::cout << "Error when load am encoder model: " << e.what() << std::endl;
        exit(0);
    }

    try {
        decoder_session_ = std::make_unique<Ort::Session>(env_, de_model.c_str(), session_options_);
        std::cout << "Successfully load model from " << de_model << std::endl;
    } catch (std::exception const &e) {
        std::cout << "Error when load am decoder model: " << e.what() << std::endl;
        exit(0);
    }
...
void Paraformer::LoadOnlineConfigFromYaml(const char* filename){
    ...
    try{
        ...
        std::cout << "Error when load argument from vad config YAML." << std::endl;
        exit(-1);
    }
}
