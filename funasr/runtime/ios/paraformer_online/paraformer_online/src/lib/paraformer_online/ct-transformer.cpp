...
void CTTransformer::InitPunc(const std::string &punc_model, const std::string &punc_config, int thread_num){
    ...
    try{
        ...
        std::cout << "Successfully load model from " << punc_model << std::endl;
    }
    catch (std::exception const &e) {
        std::cout << "Error when load punc onnx model: " << e.what() << std::endl;
        exit(0);
    }
...
vector<int> CTTransformer::Infer(vector<int32_t> input_data)
{
    ...
    {
        std::cout << "Error when run punc onnx forword: " << (e.what()) << std::endl;
        exit(0);
    }
    return punction;
}
