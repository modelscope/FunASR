...
void Audio::Disp()
{
    std::cout << "Audio time is " << (float)speech_len / MODEL_SAMPLE_RATE << " s. len is " << speech_len << std::endl;
}
...
void Audio::WavResample(int32_t sampling_rate, const float *waveform,
                          int32_t n)
{
    std::cout << "Creating a resampler:\n"
              << "   in_sample_rate: "<< sampling_rate << "\n"
              << "   output_sample_rate: " << static_cast<int32_t>(MODEL_SAMPLE_RATE) << std::endl;
    float min_freq =
