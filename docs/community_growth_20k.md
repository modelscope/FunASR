...
| `QwenLM/qwen-code#6510` pane-scoped voice dictation restore | Restores voice dictation in a 25k-star Qwen coding-agent Web Shell split-view, creating a visible local/private STT surface when the daemon advertises `voice_transcribe` | LauraGPT posted a non-blocking ASR provider-boundary note at https://github.com/QwenLM/qwen-code/pull/6510#issuecomment-4911877569: keep mic visibility capability-gated, route captured audio through the pane's own session, insert final transcript as editable draft text without auto-submit, redact provider errors, and preserve pane isolation for FunASR/SenseVoice/OpenAI-compatible STT backends. Track review and CI without treating this as a merge-ready sign-off. |
