...
| `chatmcp/mcpso#1` MCP server directory submission | Routes MCP builders browsing mcp.so toward FunASR's local speech-recognition MCP server | FunASR MCP server link was submitted to the public directory thread on 2026-07-08; monitor for ingestion into mcp.so rather than posting duplicate comments. |
...
| `QwenLM/qwen-code#6510` pane-scoped voice dictation restore | Restores voice dictation in a 25k-star Qwen coding-agent Web Shell split-view, creating a visible local/private STT surface when the daemon advertises `voice_transcribe` | LauraGPT posted a non-blocking ASR provider-boundary note at https://github.com/QwenLM/qwen-code/pull/6510#issuecomment-4911877569. Maintainer replied at https://github.com/QwenLM/qwen-code/pull/6510#issuecomment-4912022943 confirming the capability gate, editable draft/no auto-submit behavior, pane-specific transcript routing, audio-route nuance, and sanitized provider-error boundary. Track review and CI; do not add another comment unless those boundaries change. |
| `QwenLM/qwen-code#6516` VoiceButton i18n and provider-error boundary | Makes voice dictation states and retry affordances translatable in the same high-visibility Qwen Web Shell surface, which matters for non-English users running local FunASR/SenseVoice STT backends | LauraGPT posted a non-blocking note at https://github.com/QwenLM/qwen-code/pull/6516#issuecomment-4911909587: localize the visible shell states while keeping provider details as sanitized diagnostic suffixes, covering empty provider messages, no-speech notices, and safe local `/v1/audio/transcriptions` errors without leaking paths, keys, or raw upstream bodies. Track review without blocking this i18n cleanup. |
