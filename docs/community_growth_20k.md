...
| `huggingface/transformers#46180` Fun-ASR-Nano model support | Makes Fun-ASR-Nano usable through the default HF API surface and model docs | Head `fb515baa48a2` is ready for maintainer review: all 102 GitHub checks completed with 100 successes, 2 skips, 0 failures, and 0 pending; the aggregate PR CI status is green. The verified split-layout checkpoint and processor artifacts are published at Hub revision `4d5668ea`, and pinned H100 loading plus Chinese, English, and bilingual batch transcription passed with zero missing, unexpected, or mismatched keys. Avoid another status ping unless review or CI changes. |
