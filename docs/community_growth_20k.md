...
| `huggingface/transformers#46180` Fun-ASR-Nano model support | Makes Fun-ASR-Nano usable through the default HF API surface and model docs | Head `ed68b42f9ea7` is ready for maintainer review: GitHub reports the PR mergeable, all active checks are successful or intentionally skipped, and `mergeStateStatus=BLOCKED` is now the review-required gate rather than a code or CI failure. The final review cleanup keeps feature extraction in Torch through `self.pad`, uses inherited batch-capable `decode`, removes the encoder-specific initializer, and preserves the pinned H100 checkpoint evidence for single and batch Chinese/English transcription with zero missing, unexpected, or mismatched keys. Avoid duplicate pings unless a new review thread or failing check appears. |
...
| `huggingface/optimum-intel#1801` OpenVINO support | Helps CPU and edge users evaluate Fun-ASR on Intel hardware | On head `5e1da4a052c5`, 33 checks pass, 2 skip, and the dedicated `build (*asr*)` job is green. The two red jobs are unrelated: Gemma4 segfaults under the OpenVINO 2026.4 nightly, while two existing quantization tests fail with `KeyError: instruction`; all 24 review threads are resolved. Evidence is recorded at https://github.com/huggingface/optimum-intel/pull/1801#issuecomment-4955853392; wait for a maintainer rerun or new review rather than changing FunASR code. |
