...
As of 2026-07-22 19:33 UTC, the ecosystem has 35,786 combined GitHub stars, or 4,562 additional stars since the 31,224 baseline. The remaining gap to the +20,000 target is 15,438 stars by 2026-09-30, which requires roughly 221 stars/day across the remaining 70 days.
...
| `FunAudioLLM/SenseVoice` | 8,916 | 796 | 0 | 0 | 2026-07-22 |
...
| `huggingface/transformers#46180` Fun-ASR-Nano model support | Makes Fun-ASR-Nano usable through the default HF API surface and model docs | Head `5efce76908` is ready for maintainer review: GitHub reports the PR mergeable, all active checks are successful or intentionally skipped, and the only current unresolved review thread is a non-code architecture discussion where the maintainer is leaving `FunAsrNanoModel(AudioFlamingo3Model)` open for other reviewer thoughts because the adaptor is larger than a typical projector. The final review cleanup keeps feature extraction in Torch through `self.pad`, uses inherited batch-capable `decode`, removes the encoder-specific initializer, and preserves the pinned H100 checkpoint evidence for single and batch Chinese/English transcription with zero missing, unexpected, or mismatched keys. Avoid duplicate pings unless a new actionable review thread or failing check appears. |
...
- [x] Keep recent Python GitHub release pages linked to runtime assets. On 2026-07-23, `v1.3.24`, `v1.3.25`, and `v1.3.26` were verified to include a `Runtime downloads` section pointing to `runtime-llamacpp-v0.1.8`, including Linux Vulkan, Windows CUDA, all eight prebuilt llama.cpp / GGUF assets, and the matching PyPI install command. The `scripts/update_release_runtime_downloads.py` guard and `tests/test_release_runtime_downloads.py` cover future tag releases.
