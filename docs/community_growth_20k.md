...
| `Kedreamix/Linly-Dubbing#78` FunASR link refresh | Keeps a 3.2k-star multilingual video-dubbing workflow's speech-recognition docs pointed at the current official FunASR repository, preserving a relevant ASR entry for subtitle and dubbing users | PR updates the English and Chinese READMEs from `github.com/alibaba-damo-academy/FunASR` to `github.com/modelscope/FunASR`; monitor for maintainer acceptance without extra pings. |
