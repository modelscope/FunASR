...
| `krzemienski/awesome-video#102` FunClip AI video tools listing | Adds FunClip to a curated streaming/video tools list where video engineers and creator-tool builders discover AI-assisted clipping and subtitle generation workflows | PR is README-only, `+1/-0`, with FunClip placed alphabetically in `AI & Machine Learning Tools`; Sourcery review is pending, so monitor checks and maintainer feedback before adding any status comment. |
