...
| `krzemienski/awesome-video#102` FunClip AI video tools listing | Adds FunClip to a curated streaming/video tools list where video engineers and creator-tool builders discover AI-assisted clipping and subtitle generation workflows | Sourcery asked for owner/repo-style link text; updated the PR head to `f90be00` so the entry is `[modelscope/FunClip]`, still README-only and alphabetically placed. Monitor the rerun and maintainer feedback before adding any status comment. |
