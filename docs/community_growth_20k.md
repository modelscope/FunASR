...
- [x] Keep recent Python GitHub release pages linked to runtime assets. On 2026-07-23, `v1.3.21` through `v1.3.26` were verified to include a `Runtime downloads` section pointing to `runtime-llamacpp-v0.1.8`, including Linux Vulkan, Windows CUDA, all eight prebuilt llama.cpp / GGUF assets, and the matching PyPI install command. The `scripts/update_release_runtime_downloads.py` guard and `tests/test_release_runtime_downloads.py` cover future tag releases.
