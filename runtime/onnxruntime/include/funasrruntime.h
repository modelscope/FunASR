...
//OfflineStream
_FUNASRAPI FUNASR_HANDLE  	FunOfflineInit(std::map<std::string, std::string>& model_path, int thread_num, bool use_gpu=false, int batch_size=1, int module_num=1);
...
_FUNASRAPI FUNASR_RESULT	FunOfflineInfer(FUNASR_HANDLE handle, const char* sz_filename, FUNASR_MODE mode, 
											...
											int sampling_rate=16000, bool itn=true, FUNASR_DEC_HANDLE dec_handle=nullptr, int core_id=0);
